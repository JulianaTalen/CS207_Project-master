/*******************************************************************
 *  
 *  Adruino Neopixel Traffic Map
 *  
 *  This project to lights up neopixels LEDs in four locations 
 *  in New York City based on the current traffic conditions.                          
 *  Green for good, yellow for poor, and red for bad
 *  traffic condition.
 *  Traffic data is being sourced from Google Maps through 
 *  Google API. 
 *  
 *  Main Hardware:
 ...
 *  
 *  Orginal Code written by Brian Lough                                         
 *  
 *  Modify code written by Juliana Talen (200354077)
 *  for CS 207 final project 
 *  
...
String origin1 = "40.705786,-74.013393"; // Bowling Green
String destination1 = "40.704631,-74.014187"; 

String origin2 = "40.714140,-74.006304"; // City Hall Park
String destination2 = "40.712278,-74.007903"; 

String origin3 = "40.730614,-73.992475"; // New York University
String destination3 = "40.727891,-73.994739"; 

String origin4 = "40.715151,-73.989951"; // Seward Park
String destination4 = "40.714176,-73.989297"; 
...
bool checkGoogleMaps() {
  Serial.println("Getting traffic for " + origin1 + " to " + destination1);
    String responseString1 = mapsApi->distanceMatrix(origin1, destination1, "now"); // site 1
    
     Serial.println("Getting traffic for " + origin2 + " to " + destination2);
    String responseString2 = mapsApi->distanceMatrix(origin2, destination2, "now"); // site 2
    
     Serial.println("Getting traffic for " + origin3 + " to " + destination3);
    String responseString3 = mapsApi->distanceMatrix(origin3, destination3, "now");// site 3
    
     Serial.println("Getting traffic for " + origin4 + " to " + destination4);
    String responseString4 = mapsApi->distanceMatrix(origin4, destination4, "now");// site 4
    
    DynamicJsonBuffer jsonBuffer;
    JsonObject& response = jsonBuffer.parseObject(responseString1);
    if (response.success()) {
      ...
      if(responseString1 == ""){
        ...
        Serial.println(responseString1);
      }
    ...
    if (response2.success()) {
      if (response2.containsKey("rows")) {
        JsonObject& element = response2["rows"][0]["elements"][0];
        String status = element["status"];
        if(status == "OK") {
    ...
    if (response3.success()) {
      if (response3.containsKey("rows")) {
        JsonObject& element = response3["rows"][0]["elements"][0];
        String status = element["status"];
        if(status == "OK") {
    ...
    if (response4.success()) {
      if (response4.containsKey("rows")) {
        JsonObject& element = response4["rows"][0]["elements"][0];
        String status = element["status"];
        if(status == "OK") {
      ...
      if(responseString4 == ""){
        ...
        Serial.println("Failed to parse Json. Response:");
        Serial.println(responseString4);
      }
